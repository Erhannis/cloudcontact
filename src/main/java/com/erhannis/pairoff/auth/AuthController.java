...
public class AuthController {
    ...
    private static String doLogin(Request req, Response res) {
        ...
        //the email is sent here and the salt and verifier for that email is
    ...
    private static Object doAuth(Request req, Response res) {
        ...
        //it will concatenate them in the format M1:A and send it as password along the email to the server
        //the ser
    ...
    private static String doSignUp(Request req, Response res) {
        ...
        User user = new User(email, salt, verifier);
        ...
        if (i > 0) {
            ...
            response.put("code", "401");
            response.put("status", "ERROR! email exists already!");
            res.status(401);
        }
